        ...
        foreach($capacityPool in $capacityPools | Where-Object {$_.Location -eq $netAppRegion -and $_.Name.Split("/")[0] -eq $netAppAccount.Name}) {
            ...
            foreach($volume in $volumes | Where-Object {$_.Location -eq $netAppRegion -and $_.Name.Split("/")[1] -eq $capacityPool.Name.Split("/")[1]}) {
                ...
                if($errorCondition -eq 0) {
                    ...
                    Write-Host '    |--' $volume.Name.Split("/")[2]'**WARNING** (Consuming'$displayConsumed' GiB of provisioned' $volumeCapacities[$volume.ResourceId]'GiB,'$displayPercentConsumed'% consumed) Current Size:' $volumeCapacities[$volume.ResourceId]'GiB, Suggested size:'$newVolumeQuota' GiB' -ForegroundColor Yellow
